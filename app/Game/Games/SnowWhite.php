<?php

namespace App\Game\Games;

use App\Game\Games\Game;

class SnowWhite extends Game {
    public static $reelsAmount = 5;
    public static $scatter = [10, 6, 7];
    public static $joker = 5;
    public static $freeSpinsAmount = 10;
    public static $symbolsAmount = 12;
    public static $paytable = [
        [ 0, 0, 10, 40, 150 ], //PAYTABLE FOR SYMBOL 1
        [ 0, 0, 10, 40, 150 ],  //PAYTABLE FOR SYMBOL 2
        [ 0, 0, 10, 40, 150 ],  //PAYTABLE FOR SYMBOL 3
        [ 0, 0, 5, 20, 100 ],  //PAYTABLE FOR SYMBOL 4
        [ 0, 0, 5, 20, 100 ],    //PAYTABLE FOR SYMBOL 5
        [ 0, 10, 100, 1000, 5000 ],    //PAYTABLE FOR SYMBOL 6
        [ 0, 0, 0, 0, 0 ],     //PAYTABLE FOR SYMBOL 7
        [ 0, 0, 0, 0, 0 ],      //PAYTABLE FOR SYMBOL 8
        [ 0, 5, 50, 200, 1000 ],     //PAYTABLE FOR SYMBOL 9
        [ 0, 5, 15, 75, 250 ],     //PAYTABLE FOR SYMBOL 10
        [ 0, 0, 0, 0, 0 ],     //PAYTABLE FOR SYMBOL 11
        [ 0, 5, 25, 100, 500 ]    //PAYTABLE FOR SYMBOL 12
    ];

    function __construct($game) {
        $this->game = $game;
    }

    public function spin() {
        return $this->game->spin("checkForWinCombosTrippleScatter", "generateFinalSymbolsTrippleScatter");
    }

    public function bonusSpin() {
        // return $this->game->bonusSpin("FreeSpinsTrippleScatter");
    }

    public function areBonusSpins() {
        return false;
        // return $this->game->areBonusSpins();
    }

}
